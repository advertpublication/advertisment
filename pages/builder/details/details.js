...
Page({
  ...
  uploadText() {
    let params = {
      userId: wx.getStorageSync("userInfo").accountId,
    ...
    if (this.data.item.publType == "1" && this.data.myResourceArray.length == 0) {
      ...
      httpPostNew(EXPLAIN_URL, params).then(res => {
        console.log("施工结果反馈",res);
          this.setData({
            drid: res.data.drid
          });
  ...
  onLoad: function(options) {
    ...
    if (options.item && options.item != "undefined") {
      ...
      this.setData({
        ...
        // explain:JSON.parse(options.item).buildremarks
      });
    }
   ...
   */
  onShow: function() {
    //this.getBuildTime(); //得到待施工时间
    //this.getApproveTime(); //得到施工通过时间
  },
